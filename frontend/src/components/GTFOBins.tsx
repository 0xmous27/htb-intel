...
const TYPE_COLORS: Record<string, string> = { shell: 'var(--neon)', 'file-read': '#00ccff', 'file-write': '#ff9900', execute: '#ff6600', 'file-transfer': '#aa44ff' }

function normGtfo(b: any): GtfoBin {
  if (b.bin_name) {
    return {
      id: b.id,
      name: b.bin_name,
...
export default function GTFOBins() {
  ...
  const [active, setActive] = useState<GtfoBin | null>(null)
        ...
        {!active ? (
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: '1px solid #1a1a1a', flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: '0.65rem', color: TYPE_COLORS[active.type], border: `1px solid ${(TYPE_COLORS[active.type] || '#888')}44`, padding: '1px 6px' }}>{active.type}</span>
